...
class TrackTransform extends Transform {
    ...
    private TrackExtension sensorsAnalyticsExtension

    TrackTransform(Project project, TrackExtension sensorsAnalyticsExtension) {
    ...
    void _transform(Context context, Collection<TransformInput> inputs, TransformOutputProvider outputProvider, boolean isIncremental) throws IOException, TransformException, InterruptedException {
        ...
        inputs.each { TransformInput input ->
            ...
            input.directoryInputs.each { DirectoryInput directoryInput ->
                ...
                if (dir) {
                    ...
                    dir.traverse(type: FileType.FILES, nameFilter: ~/.*\.class/) {
                        File classFile ->
                            if (TrackClassModifier.isShouldModify(classFile.name)) {//过滤不需要修改的class文件
                                File modified = null
                                if (!sensorsAnalyticsExtension.disableAppClick) {
                                    //修改class文件
                                    modified = TrackClassModifier.modifyClassFile(dir, classFile, context.getTemporaryDir())
                                }
                                //将修改后的class文件放到一个HashMap对象中
                                if (modified != null) {
                    ...
                    //将输入目录下的所有class文件拷贝到输出目录，再将HashMap中的修改过的class文件拷贝到输出目录，覆盖之前拷贝的class文件
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                if (!sensorsAnalyticsExtension.disableAppClick) {
                    modifiedJar = TrackClassModifier.modifyJar(jarInput.file, context.getTemporaryDir(), true)
                }
                if (modifiedJar == null) {
